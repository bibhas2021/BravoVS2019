...
namespace EDPComponent
{
    ...
    public class VistaButton : System.Windows.Forms.UserControl
    {
        ...
        [Category("Appearance"),
         DefaultValue(true),
         Description("Sets whether the button text will be " +
                     "automatically set as tool tip or not.")]
        ...
        private void VistaButton_MouseEnter(object sender, EventArgs e)
        {
            try
            {
                ...
                if (autotooltip)
                    tp.SetToolTip(this, mText.Trim());
                else tp.SetToolTip(this, null);
            }
            catch { }
        ...
        {
            if (((e.KeyCode == Keys.Space) || (e.KeyCode == Keys.Enter)) && (firsthit))
            {
                firsthit = false;
                MouseEventArgs m = new MouseEventArgs(MouseButtons.Left, 0, 0, 0, 0);
                calledbykey = true;
