    ...
    public partial class DataGridViewEDP : System.Windows.Forms.DataGridView
    {
        ...
        private bool SumTotal()
        {
            try
            {
                ...
                {
                    if (Information.IsNumeric(this.Rows[Cnt].Cells[this.Columns.Count - 2].Value))
                        DbtTotal += Convert.ToDecimal(this.Rows[Cnt].Cells[this.Columns.Count - 2].Value);

                    if (Information.IsNumeric(this.Rows[Cnt].Cells[this.Columns.Count - 1].Value))
                        CrtTotal += Convert.ToDecimal(this.Rows[Cnt].Cells[this.Columns.Count - 1].Value);
                }
        ...
        public void ExportToExcel(string FileNm)
        {
            try
            {
                ...
                foreach (Process p1 in p)
                    p1.Kill();
                MessageBox.Show("Succesfully export to Excel in " + FileNm);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
                MessageBox.Show("Not export to Excel in " + FileNm);
            }
        }
            ...
            try
            {
                ...
                if (RowColorVariation)
                {
                    ...
                    if (CelVal.ToLower().Trim() == "buy" || CelVal.ToLower().Trim() == "purchase")
                    {
                        ...
                        TempValue = EdpCom.GetFromRegisrty("PURCHASE_WINDOW_COLOR", "Midasgold\\ColorScheme");
                        if (TempValue != null)
                    ...
                    {
                        Edpcom.EDPCommon EdpCom = new Edpcom.EDPCommon();
                        TempValue = EdpCom.GetFromRegisrty("SALES_WINDOW_COLOR", "Midasgold\\ColorScheme");
                        if (TempValue != null)
                            PColor = TempValue.Split(',');
                        else
                            PColor = "255,255,255".Split(',');
